...
type ApiArticleUpdateDTO struct {
...
func (r ApiArticleUpdateDTO) TranslateError(err error) map[string]string {
	...
	for _, e := range errs {
		...
		case "CreatedAt":
			switch tag {
			case "required":
				result["createdAt"] = "创建时间不能为空"
			}
		case "UpdatedAt":
			switch tag {
			case "required":
				result["updatedAt"] = "更新时间不能为空"
			}
		}
	}
	return result
